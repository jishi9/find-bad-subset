...
class FindBadSubset(object):
	...
	def binary_split(self):
		...
		while not q.empty():
			...
			# If the effective set of unknowns is empty, move on to the next job.
			if not unknowns:
				continue
			...
			# All included elments are good. Move them all from unknown_set to good_set.
			...
			# Some of the included elements are bad
				...
				# Only one unknown element, this is the culprit. Move it form unknown_set to bad_set.
				if len(unknowns) == 1:
					assert not (unknowns & self.bad_set)
					self.unknown_set -= unknowns
					self.bad_set |= unknowns
				# Can't tell which of the unknown elements are bad, split into two groups and enqueu.
				else:
